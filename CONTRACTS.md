...
- **Status precedence:** `status` wins; without it, only the known approved actions count; a blank `status` falls back to the action. Unknown status or action → silent exit.

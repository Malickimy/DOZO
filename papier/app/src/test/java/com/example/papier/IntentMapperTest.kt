...
class IntentMapperTest {
    ...
    fun approvedStatusWithoutUrlFallsBackToDefault() {
        ...
        assertEquals(PapierContract.DEFAULT_REVIEW_URL, result.payload)
    }
    ...
    fun canceledActionMapsToCanceledOutcome() {
        val result = IntentMapper.map(
            PaymentIntent(action = PapierContract.ACTION_INGENICO_CANCELED, txnId = "TXN-4"),
            baseUrl
        )
    ...
    fun refusedActionMapsToRefusedOutcome() {
        val result = IntentMapper.map(
            PaymentIntent(action = PapierContract.ACTION_FISERV_REFUSED, txnId = "TXN-5"),
            baseUrl
        )
    ...
    fun approvedActionMapsToQr() {
        val result = IntentMapper.map(
            PaymentIntent(action = PapierContract.ACTION_INGENICO, reviewUrl = reviewUrl),
            baseUrl
        )
        assertTrue(result is MappedState.Approved)
    }
    ...
    fun displayTimeoutIsClamped() {
        assertEquals(
            ...
            PapierConfig.clampTimeoutSeconds(0)
        )
        assertEquals(
            PapierContract.MIN_DISPLAY_TIMEOUT_SECONDS,
            PapierConfig.clampTimeoutSeconds(PapierContract.MIN_DISPLAY_TIMEOUT_SECONDS - 1)
        )
        assertEquals(15, PapierConfig.clampTimeoutSeconds(15))
        assertEquals(
            PapierContract.MAX_DISPLAY_TIMEOUT_SECONDS,
            PapierConfig.clampTimeoutSeconds(99)
        )
    }
    ...
    fun missingTxnIdFallsBackToDefault() {
        ...
        assertEquals(PapierContract.DEFAULT_TXN_ID, result.txnId)
    }
    ...
    fun approvedDismissalMapsToApprovedCode() {
        assertEquals(RESULT_APPROVED, PapierContract.resultCodeFor(null))
    }

    @Test
    fun canceledOutcomeMapsToCanceledCode() {
        assertEquals(RESULT_CANCELED, PapierContract.resultCodeFor(OutcomeKind.CANCELED))
    }

    @Test
    fun refusedOutcomeMapsToRefusedCode() {
        assertEquals(RESULT_REFUSED, PapierContract.resultCodeFor(OutcomeKind.REFUSED))
    }
